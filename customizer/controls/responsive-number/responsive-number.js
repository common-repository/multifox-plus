wp.customize.controlConstructor['mfx-responsive-number'] = wp.customize.Control.extend({
	ready: function() {
		...
		this.container.on( 'change keyup paste', 'input.mfx-responsive-input, select.mfx-responsive-select', function() {
	...
	updateValue: function() {
		...
		control.responsiveContainer.find( 'input.mfx-responsive-input' ).each( function(){
		...
		control.responsiveContainer.find( 'select.mfx-responsive-select' ).each( function(){
			var $input = jQuery(this),
				$item = $input.data( 'id' ),
				$item_value =$input.val();
