...
wp.customize.controlConstructor['mfx-background'] = wp.customize.Control.extend({
	...
	initBgControl: function(){
		...
		var control = this,
			...
			picker  = control.container.find( '.mfx-color-control' );
		...
		control.container.on( 'click', '.background-image-upload-remove-button', function( e ) {
			...
			control.container.find( '.more-settings' ).find('.message').html( multifoxPlusCustomizerControlBackground.moreSettings );
			control.container.find( '.more-settings' ).find('.icon').html( '↓' );

			if ( preview.length ) {
				preview.removeClass().addClass( 'placeholder' ).html( multifoxPlusCustomizerControlBackground.placeholder );
			}
			if ( removeButton.length ) {
				removeButton.hide();
			}
		...
		control.container.on( 'click', '.more-settings', function( e ) {
			...
			if( 'down' === jQuery(this).attr( 'data-direction' ) )
			{
				...
				jQuery(this).find('.message').html( multifoxPlusCustomizerControlBackground.lessSettings )
				jQuery(this).find('.icon').html( '↑' );
			} else {
				jQuery(this).attr('data-direction', 'down');
				jQuery(this).find('.message').html( multifoxPlusCustomizerControlBackground.moreSettings )
				jQuery(this).find('.icon').html( '↓' );
			}
		});
	},
