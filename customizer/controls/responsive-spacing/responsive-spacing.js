wp.customize.controlConstructor['mfx-responsive-spacing'] = wp.customize.Control.extend({
	ready: function(){
		'use strict';
		var control = this;

		this.container.on( 'change keyup paste', 'input[type="number"], select.mfx-responsive-select', function() {
			control.updateValue();
		});
	...
	updateValue: function() {
		...
		control.container.find( 'select.mfx-responsive-select' ).each( function(){
	...
	updateConnect: function() {
		'use strict';
		var	control = this;
		control.container.find(".mfx-spacing-connected").on("click", function(){
		...
		control.container.find(".mfx-spacing-disconnected").on("click", function(){
			var $this = jQuery( this ),
